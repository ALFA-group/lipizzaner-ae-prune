...
def evaluate_cells_lipi_simple_prune_then_eval(
    nodes: Dict[int, Node],
    epochs: int,
    visualize: str,
    ...
    **kwargs: Dict[str, Any],
    ...
    fe_cnt = 0
    for t in tqdm(range(epochs), position=1, desc="Epochs"):
        ...
        for idx in order:
            ...
            # Cell evaluation
            node = evaluate_cell_lipi_simple_prune_then_eval(
                node,
                node.learning_rate,
                iteration=t,
                visualize=visualize,
...
def evaluate_cell_lipi_simple_prune_then_eval(
    node,
    learning_rate: float,
    iteration: int,
    visualize: str,
